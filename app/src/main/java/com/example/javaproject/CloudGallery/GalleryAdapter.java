...
public class GalleryAdapter extends RecyclerView.Adapter<GalleryAdapter.ImageViewHolder> {
    ...
    private List<CloudFileModel> mImg;
    ...
    public GalleryAdapter(Context context, List<CloudFileModel> imgs) {
        mContext = context;
        mImg = imgs;
    }
    ...
    public void onBindViewHolder(@NonNull ImageViewHolder holder, int position) {
        CloudFileModel img = mImg.get(position);
        ...
        if (split[split.length - 1].equals("mp3") || split[split.length - 1].equals("mp4") || split[split.length - 1].equals("mkv") || split[split.length - 1].equals("webm")) {
            ...
            holder.mVideoView.setVideoURI(Uri.parse(img.mImageURL));
